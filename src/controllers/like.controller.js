...
const toggleTweetLike = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ message: 'An error occurred while toggling the like on the Tweet', error: error.message });
    }
});
...
const getLikedVideos = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        console.error(error);
        res.status(500).json({ message: 'An error occurred while retrieving liked videos' });
    }
});
